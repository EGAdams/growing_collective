...
export async function callMCPTool(
  ...
  try {
    ...
    if (!serverConfig) {
      throw new Error(
        `MCP server '${server}' not found in configuration. Available servers: ${Object.keys(config.mcpServers).join(", ")}`,
      );
    }
    ...
    try {
      ...
      console.log(
        `[MCP Bridge] Connected to ${server}, available tools:`,
        toolsResponse.tools?.map((t: any) => t.name).join(", ") || "none",
      );
