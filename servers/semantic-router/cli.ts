...
async function routeBatch() {
  ...
  try {
    ...
    console.log("Request".padEnd(40) + "Agent".padEnd(25) + "Confidence");
    console.log("─".repeat(80));

    results.forEach((result, i) => {
      const request = TEST_CASES[i];
      const truncated = request.length > 37 ? request.substring(0, 37) + "..." : request;

      console.log(
        truncated.padEnd(40) + result.agent.padEnd(25) + `${(result.confidence * 100).toFixed(1)}%`,
      );
    });
    ...
    console.log(
      `   High (≥90%): ${highConfidence} (${((highConfidence / results.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `   Medium (70-89%): ${mediumConfidence} (${((mediumConfidence / results.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `   Low (<70%): ${lowConfidence} (${((lowConfidence / results.length) * 100).toFixed(1)}%)`,
    );
